...
<head>
  ...
  // Normalize homepage canonical: if current controller is 'home', use site root
  $ci =& get_instance();
  $is_home_controller = false;
  if (isset($ci->router) && method_exists($ci->router, 'fetch_class')) {
    $is_home_controller = (strtolower($ci->router->fetch_class()) === 'home');
  }
  if ($is_home_controller) {
    // Use base_url for canonical root (omit index.php)
    $url = base_url();
  } else {
    $url = strtolower($url);
  }
...
href="<?= base_url('assets/images/logo/packerslogo.png')?>">
  ...
  <script type="application/ld+json">
    {
      ...
      "name": "<?= $company3?>",
      "url": "<?= site_url()?>",
      "logo": "<?= base_url()?>assets/images/logo/packerslogo.png"
    }
  </script>
  <script type="application/ld+json">
    {
      ...
      "name": "<?= $company3?>",
      "url": "<?= site_url()?>",
      "image": ["<?= base_url()?>assets/images/logo/packerslogo.png"],
      "address": {
        "@type": "PostalAddress",
        "streetAddress": "  
        <?=$address1?>",
